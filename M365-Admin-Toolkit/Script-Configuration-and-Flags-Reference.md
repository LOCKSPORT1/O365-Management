...
Covers: Offboard-HybridUser.ps1, Audit-SharedMailboxOU.ps1, Export-UserAccessProfile.ps1, New-UserFromAccessProfile.ps1, plus the shared Audit-StaleAccounts, Audit-LicenseWaste, Audit-PrivilegedAccess, Audit-SharedMailboxPermissions Updated: July 2026

1\. Folder layout

Scripts and runbooks are organized into three folders:

- access-profile/ — the four access-profile scripts and their runbooks. This document lives in docs/.
...
| Export-UserAccessProfile.ps1  | No                   | n/a - fully dynamic, reads whatever template user you point it at                 | n/a                                                  |
| New-UserFromAccessProfile.ps1 | Yes (fallbacks only) | \$Script:DefaultNewUserOU \$Script:DefaultUpnSuffix \$Script:DefaultUsageLocation | All three are CHANGE-ME placeholders                 |
...
Only one JSON file format exists in this suite: the access profile written by Export-UserAccessProfile.ps1 and read by New-UserFromAccessProfile.ps1. It is a data snapshot, not a configuration file - it captures what a specific template user's access looked like at export time, not settings that control how the scripts behave.
...
| ExportedDate  | string (timestamp)                                                             | When Export-UserAccessProfile.ps1 was run                                                     |
...
| -WhatIf  | Offboard-HybridUser.ps1, Audit-SharedMailboxOU.ps1 (with -Remediate), New-UserFromAccessProfile.ps1 | Standard PowerShell ShouldProcess preview - shows what would happen without making any change. Read-only lookups (account-existence, OU validation) still run for real since they don't modify anything. |
...
| New-UserFromAccessProfile.ps1 | SamAccountName generation, New-ADUser, local AD group provisioning (steps 1, 3, 4) | Sync wait, UsageLocation, cloud group add, license assignment                                 |
...
| -SkipEntraSyncWait     | Offboard-HybridUser.ps1, New-UserFromAccessProfile.ps1 | Skips triggering/waiting for an Entra Connect delta sync before the cloud steps run.                                |
...
Important: Audit-StaleAccounts, Audit-LicenseWaste, Audit-PrivilegedAccess, and Audit-SharedMailboxPermissions (in `entra/`, `security/`, and `exchange/`) have NO -Remediate switch at all - permanently report-only by design.
...
- Always read-only, no exceptions: Export-UserAccessProfile.ps1, Audit-StaleAccounts, Audit-LicenseWaste, Audit-PrivilegedAccess, Audit-SharedMailboxPermissions.

- Read-only by default, opt-in to write with -Remediate: Audit-SharedMailboxOU.ps1.

- Makes changes by design, supports -WhatIf to preview first: Offboard-HybridUser.ps1, New-UserFromAccessProfile.ps1.
